...
@section('content')
    ...
    @if (Auth::check())
        <div class="mx-auto  pt-10 pb-20 w-11/12 md:w-2/3">
            <div class="flex items-center">
                <form method="get" action="{{ route('recipes.feed') }}">
                @csrf
            ...
            <div class="mt-2">
                ...
                <p class="text-lg inline-block">「{{ $request }}」のレシピ{{ $recipe_count }}件</p>
                    <form method="post" action="{{ route('recipes.feed.delete') }}" class="inline-block ml-2">
                    @csrf
                    @method('delete')
            ...
            <div>
                ...
                <div class="mt-8 w-1/2 tabs">
                    ...
                    {{-- 人気順タブ --}}
                    <a href="{{ route('to.top.popular') }}" class="tab tab-lifted grow bg-[#FAFAF5] border border-1 border-[#EBEBEB] text-lg">
                        人気順
                    </a>
                </div>
    ...
    @else
        <div class="h-screen bg-[url('/img/dish.jpg')] bg-cover">
            <div class="md:flex justify-around mx-auto pt-32 w-3/4 h-screen text-center text-white text-xl">
                 <a class="block flex flex-col justify-center mx-auto w-52 lg:w-80 h-52 lg:h-80 bg-[#A5BD9F] rounded-md hover:bg-[#717F6E]" href="{{ route('to.registration.form') }}">
                   <p>未登録の方</p>
                   <p>会員登録</p>
                 </a>
                 <a class="block flex flex-col justify-center mx-auto mt-10 md:mt-0 w-52 lg:w-80 h-52 lg:h-80 bg-[#A5BD9F] rounded-md hover:bg-[#717F6E]" href="{{ route('to.login.form') }}">
                   <p>登録済みの方</p>
                   <p>ログイン</p>
                 </a>
           </div>
